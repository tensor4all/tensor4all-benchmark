...
NOTES = {
    ...
    "mpo_mpo_quantics": (
        ...
        "column is the discriminator. naive and zipup_simplett use the "
        "simplett engine with an absolute singular value cutoff; zipup_treetn "
        "and fit_treetn use the treetn engine with a relative cutoff, so which "
        "directions they keep inside that budget still differs by engine. The "
        "two zipup arms are the same algorithm on the two engines, so their "
        "difference isolates the engine. "
