...
Partition patching accepts a local SVD cutoff rather than a whole-network relative-L2 tolerance. For an `R`-node chain, the benchmark converts the requested patch-input target to `local_rtol = requested_rtol / sqrt(2 * (R - 1))`, covering one TreeTN truncation sweep's two visits per edge. Patch-only studies then compute the exact relative-L2 residual between each compressed global input and its reconstructed partition. This conversion is conservative but not advertised as a library-level error theorem; the measured residual and `patch_tolerance_met` field are authoritative. A binding hard cap may still make the requested tolerance infeasible.
...
Timed regions include only the requested elementwise product or contraction. Input generation, cache I/O, input compression, format conversion, patch preparation, exact patch-reconstruction checks, output conversion, and accuracy evaluation remain outside timing. Any numerical postprocessing required to produce the benchmarked output remains inside timing.
